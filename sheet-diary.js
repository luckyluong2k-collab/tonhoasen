...
let syncTimer,queueRunning=false;
...
function collect(){document.querySelectorAll('#rows tr[data-i]').forEach(tr=>{if(!tr.querySelector('[data-k]'))return;const i=Number(tr.dataset.i);const row=readRow(tr);if(JSON.stringify(row)!==JSON.stringify(data[i])){data[i]=row;meta[i].dirty=true;}});persist();queueStatus();scheduleSync();}
...
async function checkUnchanged(m,desired){const j=await api(`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/${encodeURIComponent(`'${TAB}'!A${m.row}:N${m.row}`)}`);if(JSON.stringify(normalize(j.values?.[0]))!==JSON.stringify(normalize(m.base))&&(!desired||JSON.stringify(normalize(j.values?.[0]))!==JSON.stringify(normalize(desired))))throw Error('Dòng trên Sheet đã thay đổi hoặc di chuyển. Bản nháp được giữ; hãy đối chiếu Sheet trước khi lưu.');}
async function saveIndex(i,automatic=false){if(saving)return;if(automatic&&(!token||!navigator.onLine))return;collect();const row=[...data[i]],m=meta[i];if(!row[0])return alert('Hãy chọn ngày.');saving=true;setProg('Đang lưu…');try{
 if(m.uncertain){setProg('Đang kiểm tra lần lưu trước trên Sheet…');await resolvePending(m,row);}
 if(m.row){await checkUnchanged(m,row);await api(`https://sheets.googleapis.com/v4/spreadsheets/${SHEET_ID}/values/${encodeURIComponent(`'${TAB}'!A${m.row}:N${m.row}`)}?valueInputOption=RAW`,{method:'PUT',body:JSON.stringify({values:[row]})});}
 ...
 m.base=row;m.dirty=JSON.stringify(data[i])!==JSON.stringify(row);m.editing=automatic;persist();if(!automatic)render();setProg('Đã lưu đúng dòng '+m.row+' trên Sheet.');
 }catch(e){if(e.notWritten&&!m.row){m.uncertain=false;delete m.attempt;persist();}setProg(e.message);}finally{saving=false;queueStatus();}}
...
function queueStatus(){const count=data.filter((r,i)=>meta[i]?.dirty&&rowHasContent(r)).length;const el=$('sheetSyncStatus');if(el)el.textContent=count?`${count} dòng đang chờ gửi · Đã giữ bản nháp trên máy. ${!navigator.onLine?'Có mạng sẽ tự gửi lại.':!token?'Bấm Kết nối Google để gửi tiếp.':'Sẽ tự gửi lên Google Sheet.'}`:'Các dòng đã nhập đã được lưu trên Google Sheet.';}
function scheduleSync(){clearTimeout(syncTimer);syncTimer=setTimeout(()=>flushQueue(),2500);}
async function flushQueue(){if(queueRunning||saving||loading||!token||!navigator.onLine){queueStatus();return;}queueRunning=true;backgroundSync=true;try{for(let i=0;i<data.length;i++){if(!token||!navigator.onLine)break;if(meta[i].dirty&&rowHasContent(data[i])){await saveIndex(i,true);if(meta[i].dirty)break;}}}finally{queueRunning=false;backgroundSync=false;queueStatus();}}
